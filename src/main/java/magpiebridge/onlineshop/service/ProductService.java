package magpiebridge.onlineshop.service;
...
import magpiebridge.onlineshop.model.product.ProductOrder;
...
public class ProductService {
  ...
  public List<ProductOrder> getOrders(User user) {
    return em
        .createQuery("SELECT o FROM ProductOrder o WHERE o.user.email = '" + user.getUsername() + "'", ProductOrder.class)
        .getResultList();
  }
  ...
  public Medication getMedication(long medicationId) {
    try {
      return em.createQuery("SELECT m FROM Medication m WHERE m.id = " + medicationId, Medication.class).getSingleResult();
